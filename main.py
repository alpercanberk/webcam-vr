...
hands = mpHands.Hands(static_image_mode=False, max_num_hands = 1, min_detection_confidence=.7)
...
prev_features = np.zeros((21, 2))
delta_pos = np.zeros((21, 2))

velocity_timesteps = 10
velocities = np.zeros((velocity_timesteps, 21, 2))
...
for i in range(ball_count):
    ...
    s_random = np.random.random_integers(40, 80)
...
for _ in range(len(HAND_CONNECTIONS)):
    connections.append(Line([0,0], [0,0], (255, 0, 0)))
...
while True:
    ...
    if results.multi_hand_landmarks:
        for handLms in results.multi_hand_landmarks:
            for id, lm in enumerate(handLms.landmark):
                ...
                features[id] = np.array([cx, cy])
                delta_pos[id] = prev_features[id] - features[id]
                prev_features[id] = features[id]
                cv2.circle(canvas, (cx, cy), 5, (0, 0, 255), cv2.FILLED)

                for i in range(len(HAND_CONNECTIONS)):
                    if(id == HAND_CONNECTIONS[i][0]):
                        connections[i].update_start([cx, cy])
                    if(id == HAND_CONNECTIONS[i][1]):
                        connections[i].update_end([cx, cy])
    ...
    cv2.putText(canvas, str(int(fps)), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3,
                (255, 255, 255), 3)

    cv2.putText(canvas, "x:" + str(int(features[8][0])), (10, 110), cv2.FONT_HERSHEY_PLAIN, 3,
                (255, 255, 255), 3)
    cv2.putText(canvas, "y:" + str(int(features[8][1])), (10, 150), cv2.FONT_HERSHEY_PLAIN, 3,
                (255, 255, 255), 3)
    cv2.putText(canvas, "v_x:" + str(int(mean_v[0])), (10, 190), cv2.FONT_HERSHEY_PLAIN, 3,
                (255, 255, 255), 3)
    cv2.putText(canvas, "v_y:" + str(int(mean_v[1])), (10, 230), cv2.FONT_HERSHEY_PLAIN, 3,
                (255, 255, 255), 3)
    ...
    for i in range(len(balls)):
        ...
        current_ball.update_hand_collision(canvas, connections, mean_v)
